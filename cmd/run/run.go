...
import (
	...
	"github.com/openfga/openfga/pkg/storage/libsql"
...
func (s *ServerContext) datastoreConfig(config *serverconfig.Config) (storage.OpenFGADatastore, encoder.ContinuationTokenSerializer, error) {
	...
	case "libsql":
		datastore, err = libsql.New(config.Datastore.URI, dsCfg)
		if err != nil {
			return nil, nil, fmt.Errorf("initialize libsql datastore: %w", err)
		}
	default:
